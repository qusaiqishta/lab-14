...
-- Query 3: Alter the books table to include a field for author id -_
 ALTER TABLE books ADD COLUMN author_id INT;
-- Query 4: Retrieves the primary key on each author and fills in the author id field in the books table --
UPDATE books SET author_id=author.id FROM (SELECT id,name FROM authors) AS author WHERE books.author=author.name;
-- Query 5: remove the extra authro column in the books table --
